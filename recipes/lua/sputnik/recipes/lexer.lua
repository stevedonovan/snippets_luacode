...
module ('sputnik.recipes.lexer',package.seeall)

local lexer = _G.sputnik.recipes.lexer
...
local function sdump(tok,options)
    if options and options.string then
		local chop,chope,_ = 2
		if tok:find '^%[' then
			_,chop = tok:find '^%[=*%['
			chop = chop+1
			chope = chop
--~ 			local i1,i2 = tok:find('^[\r\n]*',chop)
--~ 			if i1 then
--~ 				print(i1,i2,tok:sub(chop,chop))
--~ 				chope = chope + i2
--~ 			end
		else
			chope = chop
		end
        tok = tok:sub(chop,-chope)
    end
...
function lua(s,filter,options)
	filter = filter or {comments=true,space=true}
...
function expecting (tok,expected_type,no_skip_ws)
	...
	if t ~= expected_type then error ("expecting "..expected_type) end
	return v
end
